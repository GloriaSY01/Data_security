...
def CalPos(plaintext):
    ...
    print("[CALPOS] presum=",presum,end=' ')
    if plaintext in local_table:
        local_table[plaintext] += 1
        ret = random.randint(presum, presum + local_table[plaintext] - 1)
        print("plaintext exist before,range:[",presum,",",presum + local_table[plaintext] - 1,"]",end=' ')
...
def Insert(plaintext):
    ...
    print("[INSERT] text = ",plaintext,end=' ')
    print('postion = ',res)
...
def Search(left, right):
    ...
    print("[MAIN] left_pos = ",left_pos," right_pos = ",right_pos)
    # 连接数据库
    conn = pymysql.connect(host='localhost', user='user',
                           passwd='123456', database='test_db')
    cur = conn.cursor()
...
if __name__ == '__main__':
    ...
    for ciphertext in ['apple', 'pear', 'banana', 'orange', 'cherry', 'apple', 'cherry', 'orange']:
        Insert(ciphertext)
    print("[MAIN] local_table = ",local_table)
    # 假设我们搜索b和p之间的数据
    Search('b', 'p')
